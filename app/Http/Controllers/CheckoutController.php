...
class CheckoutController extends Controller
{
    public function process(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('cliente.dashboard')->with('success', 'Compra realizada con éxito. Gracias por su compra.');
        } catch (\Exception $e) {
            DB::rollback();
            $route = auth()->check() ? 'cliente.cart.index' : 'cart.index';
            return redirect()->route($route)->with('error', 'Error al procesar la compra. Por favor, inténtelo de nuevo.');
        }
    }
}