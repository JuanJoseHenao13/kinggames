...
<div class="text-center mb-12">
    <h1 class="text-4xl font-bold text-black dark:text-dorado">Carrito de compras</h1>
</div>
                    ...
                    <tr class="border-b border-gray-200 dark:border-gray-700" id="row-{{ $item->id }}">
                        <td class="py-4">
                            <div class="flex items-center">
                                ...
                                <span class="text-gray-700 dark:text-blanco">{{ $item->name }}</span>
                            </div>
                        </td>
                        <td class="text-right py-4 text-gray-700 dark:text-blanco" 
                            id="price-{{ $item->id }}">
                        ...
                        <td class="text-right py-4">
                            <div class="flex items-center justify-end quantity-controls" 
                                ...
                                <input type="text" name="cantidad" value="{{ $item->quantity }}" 
                                       ...
                                       class="w-12 text-center bg-white dark:bg-gray-700 border-t border-b border-gray-300 dark:border-gray-600" readonly>
                        ...
                        <td class="text-right py-4">
                            <form action="{{ route('cart.remove', $item->id) }}" 
                                ...
                                @method('DELETE')
                                <button type="submit" 
                                        class="text-sm bg-rojo-oscuro text-blanco font-bold py-1 px-2 rounded hover:bg-dorado hover:text-negro transition duration-300">
                                    Eliminar
                                </button>
                            </form>
                        </td>
...
@else
    <div class="text-center">
        <p class="text-xl mb-4 text-gray-700 dark:text-blanco">No hay productos en el carrito.</p>
        <a href="{{ route('productos.index') }}" 
