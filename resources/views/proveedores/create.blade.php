...
<div class="space-y-8">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 dark:from-dorado dark:to-yellow-400 bg-clip-text text-transparent">Crear Proveedor</h1>
            <p class="text-gray-700 dark:text-gray-300 mt-2">Agrega un nuevo proveedor a tu sistema</p>
        </div>
    ...
    <div class="bg-gradient-to-br from-white to-blue-50 dark:from-gris-oscuro dark:to-gray-800 rounded-2xl shadow-2xl overflow-hidden border-2 border-azul-rey/30 transition-colors duration-300">
        ...
        <form action="{{ route('proveedores.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3"
                              class="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:border-dorado focus:ring-2 focus:ring-dorado/20 bg-white dark:bg-gris-oscuro text-gray-900 dark:text-white transition-all duration-200"
                              placeholder="Describe las características del proveedor">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
