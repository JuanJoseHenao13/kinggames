...
<div class="space-y-8">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-4xl font-bold text-gray-900 dark:text-dorado">Detalle de Transacción #{{ $transaccion->id_transaccion }}</h1>
            <p class="text-gray-700 dark:text-gray-300 mt-2">Detalles de la transacción</p>
        </div>
        <a href="{{ route('transacciones.index') }}" class="group bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold py-3 px-5 rounded-2xl hover:from-blue-600 hover:to-blue-700 transition-all duration-300 transform hover:scale-105 hover:shadow-xl active:scale-95">
    ...
    <div class="bg-gradient-to-br from-white to-blue-50 dark:from-gris-oscuro dark:to-gray-800 rounded-2xl shadow-2xl border-2 border-azul-rey/30 p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                ...
                <dl class="space-y-4">
                    <div>
                        <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">ID Transacción</dt>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-lg">
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-bold rounded-full {{ $transaccion->tipo == 'venta' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                            @if($transaccion->tipo == 'venta' && $transaccion->usuario)
                                {{ $transaccion->usuario->nombre }} {{ $transaccion->usuario->apellidos }}
            ...
            <div>
                <h3 class="text-2xl font-bold text-gray-900 dark:text-dorado mb-4">Productos</h3>
                <div class="flow-root">
                            ...
                            @if($detalle->producto)
                                <li class="flex items-center py-4 space-x-4">
                                    <div class="flex-shrink-0">
                                        <img class="h-16 w-16 rounded-lg object-cover shadow-md" src="{{ asset('storage/' . $detalle->producto->imagen) }}" alt="{{ $detalle->producto->nombre }}">
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-lg font-medium text-gray-900 dark:text-white truncate">{{ $detalle->producto->nombre }}</p>
